...
const HomePage = () => {
  ...
  const { data: topStoriesData, isLoading: topStoriesLoading } = useGetTopStoriesQuery({
    queryParams: {
  ...
  const { data: showsOfWeekData, isLoading: showsOfWeekLoading } = useGetShowsOfTheWeekQuery({
    queryParams: {
  ...
  const toggleSidebar = () => {
    setShowSidebar((prev) => !prev);  // Toggle visibility
  };
  ...
  return (
    <>
      ...
      {/* Sidebar Player */}
      {showSidebar && <SidebarPlayer />}  {/* Render sidebar only if visible */}
    </>
  );
};
